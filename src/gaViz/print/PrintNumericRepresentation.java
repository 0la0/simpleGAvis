	...
	public void printPopulation (Population p, JTextArea textArea) {
		for (int i = 0; i < p.getSize(); i++) {
			...
			for (int j = 0; j < p.getNumGenes(); j++) {
				textArea.append(new String(String.format("gene %1d: %3d ", j, p.getIndividual(i).genome[j])));
			}
			textArea.append(new String(String.format("fitness %4d: ", p.getIndividual(i).rawFitness)));
			textArea.append(new String(String.format("probability %4.3f: ", p.getIndividual(i).probability)));
			textArea.append(new String(String.format("LB %4.3f: UB %4.3f: \n", p.getIndividual(i).cumulativeLowerBound, p.getIndividual(i).cumulativeUpperBound)));
		}
