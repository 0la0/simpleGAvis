...
public class AnimationDriver {
	...
	public AnimationDriver (GaConfigOptions options) {
		...
		this.imgWidth = BinaryStringHelper.maxVal;
		this.imgHeight = BinaryStringHelper.maxVal;
		if (this.options.numGenes == 3) {
			this.imgWidth = (int) (this.options.populationSize * 2);
			this.imgHeight = 1;
		}
		this.createBufferedImage(this.imgWidth, this.imgHeight);
		
		if (this.options.fitnessObj instanceof FitnessCustomGoal) {
			this.isCustomGoal = true;
		}
		this.buildAnimationFrame(500, 500);
		this.timer = new Timer(animationStepTime, actionEvent -> {
	...
	private void generate () {
		...
		if (Math.random() < 0.005) this.createNewGoal();
	...
	private void animateXY () {
		...
		this.imgGraphics.fillRect(0,  0, this.imgWidth,  this.imgHeight);
		...
		renderPopulation.getIndividuals().forEach(individual -> {
			...
			int x = (int) Math.floor(this.imgWidth * normalX);
			int y = (int) Math.floor(this.imgHeight * normalY);
	...
	private void animateRGBXY () {
		...
		this.gaGenerator.getGenerations().getPopulations().forEach(population -> {
			...
			population.getIndividuals().stream().forEach(individual -> {
				...
				int x = (int) Math.floor(this.imgWidth * normalVals[3]);
				int y = (int) Math.floor(this.imgHeight * normalVals[4]);
